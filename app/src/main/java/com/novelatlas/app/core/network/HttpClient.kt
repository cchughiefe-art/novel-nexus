package com.novelatlas.app.core.network
...
class HttpClient {
        ...
        repeat(retries + 1) { attempt ->
            try {
                ...
                client.newCall(req).execute().use { response ->
                    return response.requireBody(url)
                }
            } catch (t: Throwable) {
                last = t
    ...
    companion object {
        const val USER_AGENT =
            "Mozilla/5.0 (Linux; Android 15) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126 Mobile Safari/537.36 NovelAtlas/0.1"
    }
}
